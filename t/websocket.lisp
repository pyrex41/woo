  ...
  (testing "a declined upgrade takes the buffered octets back"
    (let ((socket (make-bare-socket)))
      ...
      (ok (equal (utf-8-bytes-to-string (woo.websocket:take-pending-websocket-data socket))
                 "GET / HTTP/1.1 tail"))
      (ok (null (woo.websocket:take-pending-websocket-data socket)))))
  (testing "more than *max-pending-websocket-octets* on one connection closes it"
    (with-stubbed-close (closed)
      (let ((woo.websocket::*max-pending-websocket-octets* 100)
            (before (woo.websocket::pending-websocket-octets))
            (socket (make-bare-socket)))
        (woo.websocket:feed-websocket-data socket (make-array 60 :element-type '(unsigned-byte 8)))
        (ok (null closed) "60 octets are held")
        (ok (= (woo.websocket::pending-websocket-octets) (+ before 60)))
        (woo.websocket:feed-websocket-data socket (make-array 41 :element-type '(unsigned-byte 8)))
        (ok (equal closed (list socket)) "101 octets close the connection")
        (ok (= (woo.websocket::pending-websocket-octets) before) "its octets are released")
        (ok (null (woo.websocket:take-pending-websocket-data socket))))))
  (testing "past *max-total-pending-websocket-octets* for all connections, the one that would exceed it is closed"
    (with-stubbed-close (closed)
      (let* ((before (woo.websocket::pending-websocket-octets))
             (woo.websocket::*max-total-pending-websocket-octets* (+ before 150))
             (a (make-bare-socket))
             (b (make-bare-socket))
             (c (make-bare-socket))
             (octets (make-array 100 :element-type '(unsigned-byte 8))))
        (woo.websocket:feed-websocket-data a octets)
        (woo.websocket:feed-websocket-data b octets)
        (ok (equal closed (list b)) "the second 100 octets pass the total")
        (ok (= (woo.websocket::pending-websocket-octets) (+ before 100)))
        (setf (woo.ev.socket:socket-open-p a) nil)
        (woo.websocket:feed-websocket-data c octets)
        (ok (equal closed (list b)) "a closed connection's octets no longer count")
        (ok (equalp (woo.websocket:take-pending-websocket-data c) octets))
        (ok (= (woo.websocket::pending-websocket-octets) before)
            "setup or decline releases them")))))
